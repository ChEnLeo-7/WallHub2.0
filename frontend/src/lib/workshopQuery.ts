...
export function workshopCacheKey(filters: Filters, page: number, pageSize: number, exactPhrase: boolean, steamDataSource = 'community') {
  const query = exactPhrase ? { filters, exactPhrase: true, steamDataSource } : { filters, steamDataSource };
  return JSON.stringify(page === 1 ? { ...query, page } : { ...query, page, pageSize });
}

export function findWorkshopCacheEntry(cache: Map<string, WorkshopCacheEntry>, filters: Filters, page: number, pageSize: number, exactPhrase: boolean, steamDataSource = 'community') {
  const exact = cache.get(workshopCacheKey(filters, page, pageSize, exactPhrase, steamDataSource));
  ...
  for (const [key, entry] of cache) {
    try {
      ...
      if (parsed.page !== page || parsed.steamDataSource !== steamDataSource || JSON.stringify(parsed.filters) !== JSON.stringify(filters) || !!parsed.exactPhrase !== !!exactPhrase) continue;
...
export function buildQuery(filters: Filters, page: number, pageSize: number, exactPhrase: boolean, nsfw = true, steamDataSource = 'community') {
  ...
  const validGenres = filters.genres.filter((genre) => [
  ...
  (filters.officialTags || []).forEach((tag) => {
    if (!tags.includes(tag)) params[`requiredtags[${requiredTagCount(params)}]`] = tag;
  });
  resolutionTags.forEach((tag) => {
    if (!tags.includes(tag)) params[`requiredtags[${requiredTagCount(params)}]`] = tag;
  });
...
function requiredTagCount(params: Record<string, string | number>) {
  return Object.keys(params).filter(key => /^requiredtags/.test(key)).length;
}
