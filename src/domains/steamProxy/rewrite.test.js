...
test('rewrite factory preserves its public tool surface', () => {
...
test('runtime client script fragments preserve generated bytes', () => {
  const tools = createTools();
  const script = tools.wallhubProxyClientScript('https://steamcommunity.com/app/431960/workshop/');

  assert.equal(script.length, 39276);
  assert.equal(
    crypto.createHash('sha256').update(script).digest('hex'),
    'b6ca1de6949fc7630e42293d64451baec49d455b9ac52c28d96fc48c41403e5a'
  );
});
