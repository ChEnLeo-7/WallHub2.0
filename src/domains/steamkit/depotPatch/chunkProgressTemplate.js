...
function writeChunkProgressSource(projectDir) {
  ...
  fs.writeFileSync(chunkProgressPath, [
    ...
    '        {',
    '            return DownloadDepotChunkWithOwnedClientAsync(client, WallHubCreateHttpClient, depotId, chunk, server, destination, depotKey, proxyServer, cdnAuthToken, onBytesRead, cancellationToken);',
    '        }',
    '',
    ...
    '            return DownloadDepotChunkWithOwnedClientAsync(client, () => steamClient.Configuration.HttpClientFactory(HttpClientPurpose.CDN), depotId, chunk, server, destination, depotKey, proxyServer, cdnAuthToken, onBytesRead, cancellationToken);',
    '        }',
    '',
    '        public static Task<int> DownloadDepotChunkAsync(this Client client, HttpClient httpClient, uint depotId, DepotManifest.ChunkData chunk, Server server, byte[] destination, byte[] depotKey = null, Server proxyServer = null, string cdnAuthToken = null, Action<int> onBytesRead = null, CancellationToken cancellationToken = default)',
    '        {',
    '            ArgumentNullException.ThrowIfNull(httpClient);',
    '            return DownloadDepotChunkCoreAsync(client, httpClient, depotId, chunk, server, destination, depotKey, proxyServer, cdnAuthToken, onBytesRead, cancellationToken);',
    '        }',
    '',
    '        private static async Task<int> DownloadDepotChunkWithOwnedClientAsync(Client client, Func<HttpClient> httpClientFactory, uint depotId, DepotManifest.ChunkData chunk, Server server, byte[] destination, byte[] depotKey, Server proxyServer, string cdnAuthToken, Action<int> onBytesRead, CancellationToken cancellationToken)',
    '        {',
    '            using var httpClient = httpClientFactory();',
    '            return await DownloadDepotChunkCoreAsync(client, httpClient, depotId, chunk, server, destination, depotKey, proxyServer, cdnAuthToken, onBytesRead, cancellationToken).ConfigureAwait(false);',
    '        }',
    '',
    '        private static async Task<int> DownloadDepotChunkCoreAsync(Client client, HttpClient httpClient, uint depotId, DepotManifest.ChunkData chunk, Server server, byte[] destination, byte[] depotKey, Server proxyServer, string cdnAuthToken, Action<int> onBytesRead, CancellationToken cancellationToken)',
    '        {',
    ...
    '            WallHubStreamTrace("WALLHUB_DEPOT_CDN_HTTP_STAGE:client-ready");',
    '',
    '            try',
    '            {',
    '                WallHubStreamTrace("WALLHUB_DEPOT_CDN_HTTP_STAGE:send-start");',
    ...
    '                WallHubStreamTrace($"WALLHUB_DEPOT_CDN_HTTP_STAGE:headers:{(int)response.StatusCode}");',
    '',
    ...
    '                WallHubStreamTrace("WALLHUB_DEPOT_CDN_HTTP_STAGE:body-start");',
    '                var firstRead = true;',
    '                while (true)',
    '                {',
    ...
    '                    {',
    '                        firstRead = false;',
    '                        WallHubStreamTrace($"WALLHUB_DEPOT_CDN_HTTP_STAGE:body-first:{read}");',
    '                    }',
