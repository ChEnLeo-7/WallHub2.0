...
function streamWorkerTemplateTail() {
  return [
    ...
    '        }',
    '',
    '        private static async Task<StreamChunkRead> DownloadStreamChunkAsync(Steam3Session steam3, CDNClientPool cdnPool, HttpClient chunkHttpClient, StreamPlan plan, DepotManifest.ChunkData chunk, CancellationToken cancellationToken)',
    '        {',
    '            var buffer = new byte[(int)chunk.UncompressedLength];',
    '            var written = await DownloadChunkAsync(steam3, cdnPool, chunkHttpClient, plan, chunk, buffer, cancellationToken).ConfigureAwait(false);',
    ...
    '        }',
    '',
    '        private static async Task<int> DownloadChunkAsync(Steam3Session steam3, CDNClientPool cdnPool, HttpClient chunkHttpClient, StreamPlan plan, DepotManifest.ChunkData chunk, byte[] destination, CancellationToken cancellationToken)',
    '        {',
    '            var failures = new List<Exception>();',
    ...
    '                {',
    '                    connection = cdnPool.GetConnection();',
    '                    WallHubStreamTrace($"WALLHUB_DEPOT_CDN_CHUNK_STAGE:{attempt}/{maxAttempts}:connection:{connection?.Host ?? "unknown"}");',
    '                    WallHubLogCdnHost(connection);',
    '                    string cdnToken = null;',
    '                    if (steam3.CDNAuthTokens.TryGetValue((plan.DepotId, connection.Host), out var authTokenCallbackPromise))',
    '                    {',
    '                        WallHubStreamTrace($"WALLHUB_DEPOT_CDN_CHUNK_STAGE:{attempt}/{maxAttempts}:auth-wait:{connection.Host}");',
    '                        var result = await authTokenCallbackPromise.Task.WaitAsync(attemptToken).ConfigureAwait(false);',
    '                        cdnToken = result.Token;',
    '                        WallHubStreamTrace($"WALLHUB_DEPOT_CDN_CHUNK_STAGE:{attempt}/{maxAttempts}:auth-ready:{connection.Host}");',
    '                    }',
    '                    WallHubStreamTrace($"WALLHUB_DEPOT_CDN_CHUNK_STAGE:{attempt}/{maxAttempts}:http-start:{connection.Host}");',
    '                    var written = await cdnPool.CDNClient.DownloadDepotChunkAsync(chunkHttpClient, plan.DepotId, chunk, connection, destination, plan.DepotKey, cdnPool.ProxyServer, cdnToken, null, attemptToken).ConfigureAwait(false);',
    '                    WallHubStreamTrace($"WALLHUB_DEPOT_CDN_CHUNK_STAGE:{attempt}/{maxAttempts}:http-done:{connection.Host}:{written}");',
    '                    if (written <= 0)',
    '                    {',
    ...
    '        }',
    '',
    '        private static void WallHubStreamTrace(string message)',
    '        {',
    '            if (Environment.GetEnvironmentVariable("DEPOTDOWNLOADER_DEBUG") == "1") Console.Error.WriteLine(message);',
    '        }',
    '',
    ...
    '        }',
    '',
    '        private static long WallHubChunkBufferBudgetBytes()',
    '        {',
    '            const long minBytes = 8L * 1024L * 1024L;',
    '            const long maxBytes = 512L * 1024L * 1024L;',
    '            const long defaultBytes = 64L * 1024L * 1024L;',
    '            var raw = (Environment.GetEnvironmentVariable("WALLHUB_DEPOT_STREAM_CHUNK_BUFFER_BYTES") ?? string.Empty).Trim();',
    '            if (long.TryParse(raw, out var bytes) && bytes >= minBytes && bytes <= maxBytes) return bytes;',
    '            return defaultBytes;',
    '        }',
    '    }',
    '}',
