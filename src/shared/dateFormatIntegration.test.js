...
const detailsRoot = path.join(projectRoot, 'frontend/src/components/dialogs/details-dialog');
const detailsSource = [
  'useDetailsDialogController.ts',
  'DetailsMetadata.tsx',
...
test('last-updated facts use the normal peer value size and compact only when the year makes the date long', () => {
  assert.match(detailsSource, /formatDateOnlyText, shouldCompactDateOnlyText/);
  ...
  assert.equal((detailsSource.match(/label=\{text\.lastUpdated\}[\s\S]*?value=\{lastUpdatedText\}[\s\S]*?allowFullValue[\s\S]*?compactFullValue=\{compactLastUpdatedText\}/g) || []).length, 1);
  assert.equal((detailsSource.match(/compactFullValue \? 'whitespace-nowrap text-\[11px\] sm:text-xs' : 'whitespace-nowrap text-xs sm:text-sm'/g) || []).length, 1);
});
